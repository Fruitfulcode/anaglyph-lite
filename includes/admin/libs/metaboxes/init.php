...
class cmb_Meta_Box {
	...
	public function register_scripts() {
		...
		if ( 3.5 <= $wp_version ) {
			...
			if ( ! is_admin() ) {
				...
				wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
					'clear'         => __( 'Clear', 'anaglyph-lite' ),
					'defaultString' => __( 'Default', 'anaglyph-lite' ),
					'pick'          => __( 'Select Color', 'anaglyph-lite' ),
					'current'       => __( 'Current Color', 'anaglyph-lite' ),
				) );
			}
		} else {
		...
		wp_localize_script( 'cmb-scripts', 'cmb_l10', apply_filters( 'cmb_localized_data', array(
			...
			'upload_file'     => __('Use this file', 'anaglyph-lite'),
			'remove_image'    => __('Remove Image', 'anaglyph-lite'),
			'remove_file'     => __('Remove', 'anaglyph-lite'),
			'file'            => __('File:', 'anaglyph-lite'),
			'download'        => __('Download', 'anaglyph-lite'),
			'ajaxurl'         => admin_url( '/admin-ajax.php' ),
			'up_arrow'        => '<span class="order-up"></span>',
			'down_arrow'      => '<span class="order-down"></span>',
			'check_toggle'    => __( 'Select / Deselect All', 'anaglyph-lite' ),
		) ) );
...
function cmb_metabox_form( $meta_box, $object_id, $echo = true ) {
	...
	$form = sprintf( $form_format, $meta_box['id'], $object_id, $form, __( 'Save', 'anaglyph-lite' ) );
